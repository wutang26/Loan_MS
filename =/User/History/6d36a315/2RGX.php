...
class LoanApprovalController extends Controller
{
        ...
        //Loan Calcultor 
    //     $emi = LoanCalculator::emi(
    //     $loan->loan_amount,
    //     $loan->interest_rate,
    //     $loan->tenure_months
    // );

    //User enters amount → preview EMI before submitting loan.
    public function previewEMI(Request $request)
    {
    $emi = LoanCalculator::emi(
        $request->amount,
        $request->rate,
    ...
    public function create()
    {
            ...
            public function disburseLoan(Loan $loan)
            {
             ...
             public function disbursedList()
                   ...
                   public function repayments()
                    {
                        ...
                        // Admin: see all users who have repayments
                        if ($authUser->hasAnyRole(['super-admin', 'admin'])) {

                            $users = User::whereHas('loans.repayments')
                                ->with(['loans.repayments'])
                                ->paginate(1); // 👈 one user per page
                        }
                        ...
                        return view('admin.loans.repayments', compact('users'));
                    }
                ...
                public function payInstallment($repaymentId)
                        ...
                        // Update loan totals
                        $loan->increment('amount_paid', $repayment->amount);
        ...
        public function repaymentSchedule(Loan $loan)
        {
            ...
            // Admins see all loans, normal users see only their loans
            if ($user->hasAnyRole(['super-admin', 'admin'])) {
                $loans = Loan::with(['repayments', 'user'])
                    ->where('application_status', 'disbursed')
                    ->latest()
                    ->paginate(1); // paginate before get()

                    // ->get()->paginate(1); // <-- 1 user per page;
            } else {
            ...
            // Automatically create repayments for loans without schedules
        ...
        return view('admin.loans.repayment_schedule', compact('loans'));
    }
